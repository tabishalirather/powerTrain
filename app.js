function raiseToPower() {
	...
	// var numberInput = 1
	var number = BigInt(numberInput.value);
	var originalNumber = BigInt(number);
	var j = 0;
	...
	while (parseInt(number) / 9 > 1) {
		...
		if (digitsArray.length % 2 === 0) {
			...
			for (var i = 0; i < digitsArray.length; i += 2) {
				...
				result *= BigInt(tempArray[tempArray.length - 1]);
			}

			var newNum = BigInt(result);
			outputDiv.innerHTML += "---------------" + "<br>";

			outputDiv.innerHTML += "Obtained number is: " + newNum + "<br>";
			number = BigInt(newNum);
		} else {
